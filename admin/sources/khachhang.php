...
switch($act){
	...
	case "man_cat":
		get_cats();
		$template = "khachhang/cats";
		break;
	case "add_cat":
		$template = "khachhang/cat_add";
		break;
	case "edit_cat":
		get_cat();
		$template = "khachhang/cat_add";
...
function get_items(){
	...
	$sql_sp = "SELECT id,tinnoibat FROM table_khachhang where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	...
	if($spdc1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_khachhang SET tinnoibat ='".$tinnoibat."' WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_khachhang SET tinnoibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		} redirect("index.php?com=khachhang&act=man");} else transfer("Không nhận được dữ liệu", "index.php?com=khachhang&act=man");
}
//===========================================================
function get_cats(){
	global $d, $items, $paging;
	$sql = "select * from #_khachhang_item order by stt";
	$d->query($sql);
	$items = $d->result_array();
	
	$curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
	$url="index.php?com=khachhang&act=man_cat";
	$maxR=20;
	$maxP=4;
	$paging=paging($items, $url, $curPage, $maxR, $maxP);
	$items=$paging['source'];
...
function get_cat(){
	...
	$id = isset($_GET['id']) ? themdau($_GET['id']) : "";
	if(!$id)
	transfer("Không nhận được dữ liệu", "index.php?com=khachhang&act=man_cat");
	
	$sql = "select * from #_khachhang_item where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=khachhang&act=man_cat");
	$item = $d->fetch_array();
}
...
function save_cat(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=khachhang&act=man_cat");
	$id = isset($_POST['id']) ? themdau($_POST['id']) : "";
	if($id){
		$data['ten_vi'] = $_POST['ten_vi'];
		$data['ten_en'] = $_POST['ten_en'];
		...
		$data['hienthi'] = isset($_POST['hienthi']) ? 1 : 0;		
		$data['ngaysua'] =time();	
		$d->setTable('khachhang_item');
		$d->setWhere('id', $id);
		if($d->update($data))
			redirect("index.php?com=khachhang&act=man_cat&curPage=".$_REQUEST['curPage']."");
		else
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=khachhang&act=man_cat");
	}else{
		$data['ten_vi'] = $_POST['ten_vi'];
		$data['ten_en'] = $_POST['ten_en'];
		$data['tenkhongdau'] = changeTitle($_POST['ten_vi']);
		...
		$d->setTable('khachhang_item');
		if($d->insert($data))
			redirect("index.php?com=khachhang&act=man_cat");
		else
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=khachhang&act=man_cat");
	}
}
...
function delete_cat(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();		
		$sql = "delete from #_khachhang_item where id='".$id."'";
		if($d->query($sql))
			redirect("index.php?com=khachhang&act=man_cat");
		else
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=khachhang&act=man_cat");
	}else transfer("Không nhận được dữ liệu", "index.php?com=khachhang&act=man_cat");
}
?>