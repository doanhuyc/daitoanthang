...
function get_items(){
	...
	$sql_sp = "SELECT id,tinnoibat FROM table_duan where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	...
	if($spdc1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_duan SET tinnoibat ='".$tinnoibat."' WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_duan SET tinnoibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
...
function get_cats(){
	global $d, $items, $paging;
	$sql = "select * from #_duan_item order by stt";
	$d->query($sql);
	$items = $d->result_array();
	
	$curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
	$url="index.php?com=duan&act=man_cat";
	$maxR=20;
	$maxP=4;
	$paging=paging($items, $url, $curPage, $maxR, $maxP);
	$items=$paging['source'];
...
function save_cat(){
	...
	if($id){
		...
		$data['hienthi'] = isset($_POST['hienthi']) ? 1 : 0;		
		$data['ngaysua'] =time();	
		$d->setTable('duan_item');
		$d->setWhere('id', $id);
		if($d->update($data))
			redirect("index.php?com=duan&act=man_cat&curPage=".$_REQUEST['curPage']."");
		else
...
function delete_cat(){
	...
	}else transfer("Không nhận được dữ liệu", "index.php?com=duan&act=man_cat");
}
?>