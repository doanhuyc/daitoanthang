<?php
	...
	if(isset($_POST['cmd'])){
		if($_POST['cmd']=='add_comment'){
			...
			$ngaydangky=time();
			$d->reset();
			$sql = "INSERT INTO  table_nhanxet_hoidap (id_pro,ten,noidung,ngaytao, hienthi) 
			...
			$ngaydangky=time();
			$d->reset();
			$sql = "INSERT INTO  table_nhanxet_hoidap (id_pro,id_parent,ten,noidung,ngaytao, hienthi) 
			...
			$d->reset();
			$sql= "select * from #_nhanxet_hoidap where hienthi=1 and id_pro='".$id_pro."' and id_parent=0 order by ngaytao desc, id desc";
			$d->query($sql);
			...
			if(!empty($nhanxet)){
                ...
                <?php
					$d->reset();
					$sql= "select * from #_nhanxet_hoidap where hienthi=1 and id_parent='".$nhanxet_item['id']."' and id_pro='".$id_pro."' order by ngaytao desc, id desc";
					$d->query($sql);
