...
<script language="javascript">
	function del(pid){
		if(confirm('Do you really mean to delete this item')){
			...
			document.form3.command.value='delete';
	...
	function update_cart(){
		document.form3.command.value='update';
		document.form3.submit();
	}
</script>
<div class="tcat"><?=_giohang?></div>
	<div class="content">
...
<input type="hidden" name="command" /> 
    	...
    	<?
			if(is_array($_SESSION['cart'])){
            	echo '<tr bgcolor="#075992" style="font-weight:bold;color:#FFF"><td align="center">STT</td><td>'._tengh.'</td><td align="center">'._gia.'</td><td align="center">'._soluong.'</td><td align="center">'._tonggia.'</td><td align="center">'._xoa.'</td></tr>';
				$max=count($_SESSION['cart']);
				for($i=0;$i<$max;$i++){
					$pid=$_SESSION['cart'][$i]['productid'];
			...
			?>
                ...
                <b><?=_tonggia?>: <?=number_format(get_order_total(),0, ',', '.')?>&nbsp;VNĐ</b>
                </td></tr>
                <tr>
                	<td colspan="6" align="right">
                     <input type="button" value="<?=_muatiep?>" onclick="window.location='http://<?=$config_url?>/san-pham.html'" class="button">
                <input type="button" value="<?=_xoatatca?>" onclick="clear_cart()" class="button">
                <input type="button" value="<?=_capnhat?>" onclick="update_cart()" class="button"><input type="button" value="<?=_thanhtoan?>" onclick="window.location='http://<?=$config_url?>/thanh-toan.html'" class="button">
                    </td>
                </tr>
			<?
            }
